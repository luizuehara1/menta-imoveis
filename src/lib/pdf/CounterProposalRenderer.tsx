...
interface CounterProposalRendererProps {
  contract: any;
  company: any;
}

export const CounterProposalRenderer: React.FC<CounterProposalRendererProps> = ({ contract, company }) => {
  ...
  // Resolve the accepting party (vendedor)
  ...
  const blocks: BlockDefinition[] = [
        ...
        <div className="w-full">
          ...
          <div className="text-center my-4">
            <h1 className="text-sm font-bold uppercase tracking-wider text-slate-950">
              CONTRAPROPOSTA DE COMPRA E VENDA DE IMÓVEL
            </h1>
          </div>
      ...
      render: () => (
        <div className="section-block mb-4 w-full">
          <h3 className="text-[11.5pt] font-bold text-slate-950 uppercase tracking-wider border-b border-slate-300 pb-0.5 mb-2">
            I - PROPRIETÁRIO VENDEDOR (PROPONENTE DA CONTRAPROPOSTA)
          </h3>
          <div className="grid grid-cols-2 gap-x-6 gap-y-1 text-[11.5pt] leading-normal">
            <p><strong>Nome Completo:</strong> {safeText(contraParteAceitante.nome || vend.nome || 'Não informado')}</p>
            <p><strong>CPF:</strong> {safeText(contraParteAceitante.cpf || vend.cpf || 'Não informado')}</p>
            <p><strong>RG:</strong> {safeText(contraParteAceitante.rg || vend.rg || 'Não informado')}</p>
            <p><strong>Estado Civil:</strong> {safeText(contraParteAceitante.estadoCivil || vend.estadoCivil || 'Não informado')}</p>
            <p><strong>Telefone:</strong> {safeText(contraParteAceitante.telefone || vend.telefone || 'Não informado')}</p>
            <p><strong>Profissão:</strong> {safeText(contraParteAceitante.profissao || vend.profissao || 'Não informado')}</p>
            <p className="col-span-2"><strong>E-mail:</strong> {safeText(contraParteAceitante.email || vend.email || 'Não informado')}</p>
            <p className="col-span-2"><strong>Endereço:</strong> {formatFullAddress(contraParteAceitante || vend)}</p>
          </div>
        </div>
      )
    },
    {
      ...
      render: () => (
        <div className="section-block mb-4 w-full">
          <h3 className="text-[11.5pt] font-bold text-slate-950 uppercase tracking-wider border-b border-slate-300 pb-0.5 mb-2">
            II - DESTINATÁRIO COMPRADOR
          </h3>
          <div className="grid grid-cols-2 gap-x-6 gap-y-1 text-[11.5pt] leading-normal">
            <p><strong>Nome Completo:</strong> {safeText(comp.nome || contract.nomeCliente || 'Não informado')}</p>
            <p><strong>CPF/CNPJ:</strong> {safeText(comp.cpfCnpj || comp.cpf || 'Não informado')}</p>
            <p><strong>RG:</strong> {safeText(comp.rg || 'Não informado')}</p>
            <p><strong>Estado Civil:</strong> {safeText(comp.estadoCivil || 'Não informado')}</p>
            <p><strong>Telefone:</strong> {safeText(comp.telefone || 'Não informado')}</p>
            <p><strong>Profissão:</strong> {safeText(comp.profissao || 'Não informado')}</p>
            <p className="col-span-2"><strong>E-mail:</strong> {safeText(comp.email || 'Não informado')}</p>
            <p className="col-span-2"><strong>Endereço:</strong> {formatFullAddress(comp)}</p>
          </div>
        </div>
      )
    },
  ...
  if (listClauses.length > 0 || customClausesText) {
    ...
    if (clausesToRender.length > 0) {
      ...
      blocks.push({
        id: `counter_clause_heading_and_0`,
        render: () => (
        ...
        <div className="grid grid-cols-2 gap-x-8 gap-y-6 text-center mt-4">
          <div className="flex flex-col items-center">
            <div className="w-full border-t border-slate-400 pt-1 text-slate-900 font-bold text-[11.5pt] truncate">
              {safeText(contraParteAceitante.nome || vend.nome || 'PROPRIETÁRIO VENDEDOR')}
            </div>
          ...
          <div className="flex flex-col items-center">
            <div className="w-full border-t border-slate-400 pt-1 text-slate-900 font-bold text-[11.5pt] truncate">
              {safeText(comp.nome || contract.nomeCliente || 'PROPONENTE COMPRADOR')}
            </div>
